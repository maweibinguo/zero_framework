...
class ArticleTagModel extends BaseModel
{
    ...
    public function deleteArticleFromTargetTag($article_id, $tag_name)
    {
        ...
        if($new_scores <= 0.0) {
            ...
            if($deleted_number <= 0) {
                throw new \Exception('从标签集合中删除标签失败');
            }
        }
        ...
        if(empty($result)) {
            throw new \Exception('删除指定标签下的文章失败');
        }
    }
    ...
    public function addArticleToTag($article_id, $tag_name, $scores)
    {
        ...
        //将文章添加到这个标签下
        $key_name = $this->getKeyName([ static::TAG_OF_ARTICLE, $tag_name]);
        $new_number = static::$redis->zAdd($key_name, $scores, $tag_name);
        if($new_number <= 0) {
            throw new \Exception('向指定标签下添加文章失败');
        }
    }
    ...
    public function getArticleListByTag($condition)
    {
        ...
        $key_name = $this->getKeyName([ static::TAG_OF_ARTICLE, $condition['tag_name'] ]);
