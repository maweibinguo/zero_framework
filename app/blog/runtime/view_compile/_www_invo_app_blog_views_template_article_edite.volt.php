...
<form class="form-horizontal" role="form" action="<?php echo $action_url;?>">
  ...
  <div class="form-group">
    <label for="ishot" class="col-sm-1">今日推荐:</label>
    <div class="col-sm-7">
        <select class="form-control" name="ishot" id="ishot">
            <option value=""/>--请选额--</option>
            <option value='1' <?php if(isset($article_detail['status']) && $article_detail['status'] == 1) { echo "selected=true"; }?> >是</option>
            <option value='0' <?php if(isset($article_detail['status']) && $article_detail['status'] == 0) { echo "selected=true"; }?> >否</option>
        </select>
    </div>
  </div>
    ...
    <div class="col-sm-7">
        <select class="form-control" name="category" id="category">
            ...
            <?php foreach($category_list as $value => $name) { ?>
                <option value='<?php echo $value;?>' <?php if(isset($article_detail['category']) && $article_detail['category'] == $value) { echo 'selected = "true"'; }?> ><?php echo $name;?></option>
            <?php }?>
        </select>
    </div>
  </div>
