...
class MottoController extends BaseController
{
     ...
     * 添加格言
     */
    public function mottoAction()
    {
        try{
            if($this->request->isPost()) {
                $motto_content = $this->request->getPost('motto_content');   
                if(empty($motto_content)) {
                    throw new \Exception("格言为空");
                }
                $motto_model = new MottoModel();
                $motto_model->addMotto($motto_content);
                $this->flashSession->success('格言添加成功');
                $this->view->setVar('issuccess', true);
            } else {
                //do nothing 
            }
            ...
            $this->flashSession->error("添加格言失败"); 
